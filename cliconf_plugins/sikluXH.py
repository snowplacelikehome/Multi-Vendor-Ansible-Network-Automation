...
name: sikluXH
short_description: Use Siklu EtherHaul/MultiHaul cliconf to run commands on Siklu platform
description:
  - This Siklu EH/MH plugin provides low level abstraction apis for
    sending and receiving CLI commands from Siklu network devices.
'''
...
class Cliconf(CliconfBase):
    ...
    def get_device_info(self):
        ...
        device_info['network_os'] = 'sikluXH'
        reply = self.get('show inventory')
        data = to_text(reply, errors='surrogate_or_strict').strip()

        match = re.search(r'inventory 1 sw-rev\s*:\s*(\S+)', data)
        if match:
            device_info['network_os_version'] = match.group(1)

        match = re.search(r'inventory 1 model-name\s*:\s*(\S+)', data)
        if match:
    ...
    def get_config(self, source='running', flags=None):
        ...
        if source == 'running':
            ...
            cmd = 'copy startup-configuration display'
